...
class CTLSTM(nn.Module):
    ...
    def forward(self, event_seqs, duration_seqs, batch_first=True, save=True):
        if batch_first:
    ...
    def get_candidates(self, mx, granularity, n_event):
        time_candidates = np.linspace(0, mx, round(mx * granularity))[1:]
        # return candidates[1:] - candidates[:-1]
        candidates = []
        for _e in range(n_event):
            candidates += [(_t_candidates, _e) for _t_candidates in time_candidates]
        return candidates

    def pred(self, time_in_seq, event_in_seq, T, total_time_seqs, seqs_length, n_type, granularity=10, batch_first=True):
        duration_in_seq = torch.cat((torch.zeros(time_in_seq.shape[0]).unsqueeze(-1), (time_in_seq[:, 1:] - time_in_seq[:, :-1])), dim=1)
        batch_size = event_in_seq.shape[0]
        seq_length = event_in_seq.shape[1]
        if batch_first:
            event_in_seq = event_in_seq.transpose(0, 1)
            # duration_in_seq = duration_in_seq.transpose(0, 1)
            time_in_seq = time_in_seq.transpose(0, 1)

        pred_time, pred_mark = [], []
        with torch.no_grad():
            for i in range(batch_size):
                candidates = self.get_candidates(T, granularity, n_type)
                time_seq = time_in_seq[:, i]
                event_seq = event_in_seq[:, i]
                likeli = []
                for candidate in candidates:
                    t, e = candidate
                    seqs_length = torch.tensor([len(time_seq)])
                    sim_time_seq = torch.cat((time_seq, torch.tensor([t]))).unsqueeze(0)
                    sim_index_seq = torch.cat((event_seq, torch.tensor([e]))).unsqueeze(0)
                    total_time_seq = torch.sum(sim_time_seq).unsqueeze(0)
                    likeli += [self.log_likelihood(event_seq.unsqueeze(0), sim_time_seq, sim_index_seq, total_time_seq, seqs_length, batch_first=True)]
                best_candidate = np.argmax(likeli)
                pred_time += [candidates[best_candidate][0]]
                pred_mark += [candidates[best_candidate][1]]

        return torch.tensor(pred_time), torch.tensor(pred_mark)

        # with torch.no_grad():
        #     # for _event_in_seq, _time_in_seq in zip(event_in_seq, time_in_seq):
        #     self.init_states(event_in_seq.shape[0])
        #     output = self(event_in_seq, duration_in_seq, save=False)
        #
        #     h, c, c_bar, o, delta = torch.chunk(output, 5, 0)
        #     # L * B * H
        #     h = torch.squeeze(h, 0)
        #     c = torch.squeeze(c, 0)
        ...
        #
        #     h_d_list = []
        #     # next_time_candidates_list = []
        #
        #     for i in range(batch_size):
        #         # next_duration_candidates = self.get_candidates(T, granularity).repeat((batch_size, 1))
        #         # next_duration_candidates.shape = 290
        #         next_duration_candidates = self.get_candidates(T, granularity)
        #         # next_time_candidates_list.append(next_duration_candidates)
        #         time_seqs = time_in_seq[:, i]
        #         event_seqs = event_in_seq[:, i]
        #         sim_time_seqs, sim_index_seqs = utils.generate_sim_time_seqs(time_seqs, seqs_length)
        #         likelihood = self.log_likelihood(event_seqs, sim_time_seqs, sim_index_seqs, total_time_seqs,
        #                                           seq_length+1)
        #
        #         _, h_d_i = self.decay(c[i], c_bar[i], o[i], delta[i], next_duration_candidates)
        #         h_d_list.append(h_d_i)
        #
        #     h_d = torch.stack(h_d_list)
        #
        #     # sim_lambda_k = F.softplus(self.wa(h_d)).transpose(0, 1)
        #     sim_lambda_k = F.softplus(self.wa(h_d))
        #     pred_incr = torch.empty(sim_lambda_k.shape[0])
        #     pred_mark = torch.empty(sim_lambda_k.shape[0])
        #     for i, _sim_lambda_k in enumerate(sim_lambda_k):
        #         max_mark = (_sim_lambda_k==torch.max(_sim_lambda_k)).nonzero()[0]
        #         # pred_incr[i] = next_time_candidates_list[i][0][max_mark[0]].item()
        #         pred_incr[i] = next_time_candidates_list[i][max_mark[0]].item()
        #         pred_mark[i] = max_mark[1].item()
        #
