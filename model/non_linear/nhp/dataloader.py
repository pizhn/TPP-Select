...
class CTLSTMDataset(Dataset):
    ...
    def __init__(self, timestamps_seq, mark_seq):
        self.event_seqs = []
        self.time_seqs = []

        for timestamps, mark in zip(timestamps_seq, mark_seq):
            self.event_seqs.append(torch.LongTensor(mark))
            self.time_seqs.append(torch.FloatTensor(np.concatenate((np.zeros(1), (timestamps[1:] - timestamps[:-1])))))

        self.n_mark = len(np.unique([m for mark in mark_seq for m in mark]))
        self.tmax = np.max([t for timestamps in timestamps_seq for t in timestamps])
