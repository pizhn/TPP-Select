...
from model.non_linear.sahp.models.sahp import SAHP
from model.non_linear.sahp.utils import atten_optimizer
from model.non_linear.sahp.utils import util
from model.non_linear.sahp.utils import evaluation
...
class MaskBatch():
    "object for holding a batch of real_data with mask during training"
...
# class ExoMaskBatch():
#     "object for holding a batch of real_data with mask during training, with exogenous messages"
#     def __init__(self, src, endo_mask, pad, device):
#         self.src = src
#         self.src_mask = self.make_std_mask(self.src, pad, device)
#         print(1)
#
...
#         # torch.cuda.set_device(device)
#         tgt_mask = (tgt != pad).unsqueeze(-2)
#         tgt_mask = tgt_mask & Variable(subsequent_mask(tgt.size(-1)).type_as(tgt_mask.real_data)).to(device)
#         return tgt_mask
...
def train_eval_sahp_exo(params):

    args, process_dim, device, tmax, \
    train_seq_times, train_seq_types, train_seq_lengths, \
    dev_seq_times, dev_seq_types, dev_seq_lengths, \
    test_seq_times, test_seq_types, test_seq_lengths, train_endo_mask, \
    batch_size, epoch_num, use_cuda = params
    ...
    for epoch in range(epoch_num):
        ...
        for i_batch in tr_loop_range:
            ...
            batch_onehot, batch_seq_times, batch_dt, batch_seq_types, batch_endo_mask, _, _, _, batch_seq_lengths = \
                util.get_batch_exo(batch_size, i_batch, model, train_seq_lengths, train_seq_times, train_seq_types, \
                                   train_endo_mask, rnn=False)

            batch_seq_types = batch_seq_types[:, 1:]

            masked_seq_types = MaskBatch(batch_seq_types, pad=model.process_dim, device=device)# exclude the first added even
            model.forward(batch_dt, masked_seq_types.src, masked_seq_types.src_mask)
            nll = model.compute_loss_exo(batch_seq_times, batch_onehot, batch_endo_mask)
...
def prediction(device, model, test_seq_lengths, test_seq_times, test_seq_types, test_size, tmax):
    model.eval()
    test_data = (test_seq_times, test_seq_types, test_seq_lengths)
    incr_estimates, incr_errors, types_real, types_estimates, probs = \
        evaluation.predict_test(model, *test_data, pad=model.process_dim, device=device,
                                hmax=tmax, use_jupyter=False, rnn=False)
    if device != 'cpu':
        incr_errors = [incr_err.item() for incr_err in incr_errors]
    ...
    return incr_estimates, incr_errors, types_real, types_estimates, probs
