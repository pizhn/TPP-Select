...
from model.non_linear.sahp.models.embedding.event_type import TypeEmbedding
from model.non_linear.sahp.models.embedding.position import PositionalEmbedding,BiasedPositionalEmbedding
from model.non_linear.sahp.models.embedding.event_embedding import EventEmbedding
from model.non_linear.sahp.models.attention.multi_head import MultiHeadedAttention
from model.non_linear.sahp.models.utils.sublayer import SublayerConnection
from model.non_linear.sahp.models.utils.feed_forward import PositionwiseFeedForward
from model.non_linear.sahp.models.base import SeqGenerator, predict_from_hidden
from model.non_linear.sahp.models.utils.gelu import GELU
...
class SAHP(nn.Module):
    ...
    def compute_loss_exo(self, seq_times, seq_onehot_types, seq_endo_mask, n_mc_samples = 20):
        """
        Compute the negative log-likelihood as a loss function.
        ...
        seq_mask = seq_onehot_types[:, 1:]
        # log_sum = (log_intensities * seq_mask).sum(dim=(2, 1))  # shape batch
        log_sum = log_intensities * seq_mask  # shape batch
        log_sum[~seq_endo_mask] = 0.
        log_sum = log_sum.sum(dim=(2, 1))
        ...
        total_intens_samples = intens_at_samples.sum(dim=2)  # shape batch * N * MC
        partial_integrals = dt_seq * total_intens_samples.mean(dim=2)
        partial_integrals[~seq_endo_mask] = 0
    ...
    def read_predict(self, seq_times, seq_types, seq_lengths, pad, device,
        ...
        """
        Read an event sequence and predict_rmtpp the next event time and type.
        ...
        if length == 0:
            # estimate_dt, next_dt, error_dt, next_type, estimate_type
        ...
        from model.non_linear.sahp.train_functions.train_sahp import MaskBatch
        ...
        with torch.no_grad():
            ...
            if self.omega.shape[1] == 0:  # only one element
                ...
                return estimate_dt, next_dt, error_dt, next_type, estimate_type, None
