import numpy as np
import tqdm
from model.non_linear.sahp.models.base import SeqGenerator
...
def predict_test(model, seq_times, seq_types, seq_lengths, pad, device='cpu',
    ...
    for index_ in index_range_:
        _seq_data = (seq_times[index_],
                     ...
                     seq_lengths[index_])
        if rnn:
            est, real_dt, err, real_type, est_type, estimate_type_prob = model.read_predict(*_seq_data, hmax)
        else:
            est, real_dt, err, real_type, est_type, estimate_type_prob = model.read_predict(*_seq_data, pad, device, hmax)
        ...
        estimate_type_probs.append(estimate_type_prob)
    ...
    return incr_estimates, incr_errors, types_real, types_estimates, estimate_type_probs
