...
from model.non_linear.thp.transformer.Models import get_non_pad_mask
...
def log_likelihood_exo(model, data, time, types, endo_mask):
    """ Log-likelihood of sequence. """

    exo_mask = torch.logical_not(endo_mask)
    non_pad_mask = get_non_pad_mask(types).squeeze(2)
    ...
    # non_event_ll = compute_integral_biased(type_lambda, time, non_pad_mask)
    non_event_ll = compute_integral_unbiased(model, data, time, non_pad_mask, type_mask)
    non_event_ll[exo_mask[:, 1:]] = 0
    non_event_ll = torch.sum(non_event_ll, dim=-1)

    return event_ll, non_event_ll


def type_loss_exo(prediction, types, loss_func, endo_mask):
    """ Event prediction loss, cross entropy or label smoothing. """
    ...
    loss = torch.sum(loss[endo_mask[:, 1:]])
    return loss, correct_num


def time_loss_exo(prediction, event_time, endo_mask):
    """ Time prediction loss. """
    ...
    # event time gap prediction
    diff = (prediction - true)[endo_mask[:, 1:]]
    se = torch.sum(diff * diff)
    return se

def time_loss_ae(prediction, event_time):
    # absolute error
    prediction.squeeze_(-1)
...
class LabelSmoothingLoss(nn.Module):
    ...
    and p_{prob. computed by transformer}(w) is minimized.
    """
