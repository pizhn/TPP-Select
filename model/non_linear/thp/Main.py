...
def train_with_exo_mae(model, training_data, validation_data, optimizer, scheduler, pred_loss_func, opt):
    """ Start training. """
    ...
    likelihood, mark_error, time_error = eval_epoch_mae(model, validation_data, pred_loss_func, opt)
    print('  - (Testing)     loglikelihood: {likelihood: 8.5f}, '
          'mark error: {mark_error: 8.5f}, time error: {time_error: 8.5f}, '
          'elapse: {elapse:3.3f} min'
          .format(likelihood=likelihood, mark_error=mark_error, time_error=time_error,
                  elapse=(time.time() - start) / 60))
...
# train epoch but returns mae
def train_epoch_mae(model, training_data, optimizer, pred_loss_func, opt):
    """ Epoch operation in training phase. """
...
def eval_epoch_mae(model, validation_data, pred_loss_func, opt, auc):
    """ Epoch operation in evaluation phase. """
...
def train_mae(model, training_data, validation_data, optimizer, scheduler, pred_loss_func, opt):
    """ Start training. """
    for epoch_i in range(opt.epoch):
        epoch = epoch_i + 1
        print('[ Epoch', epoch, ']')
        ...
        start = time.time()
        likelihood, mark_error, time_error = train_epoch_mae(model, training_data, optimizer, pred_loss_func, opt)
        print('  - (Training)    loglikelihood: {likelihood: 8.5f}, '
              'mark error: {mark_error: 8.5f}, time error: {time_error: 8.5f}, '
              'elapse: {elapse:3.3f} min'
              .format(likelihood=likelihood, mark_error=mark_error, time_error=time_error,
